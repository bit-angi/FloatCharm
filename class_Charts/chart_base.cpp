    ...
    if(showAll)
    {
        ...
        else
        {
            if(chartText.tmpEdit2)
            {
                ...
                chartText.textType2->setTextColor(color);
...
void Chart_Base::updateWidgetPosInof()
{
    ...
    // 考虑旋转的影响
    QRectF rotatedRect = QTransform().mapRect(QRectF(x, y, w, h));

    // 将 qreal 转换为 int
    int newX = static_cast<int>(rotatedRect.left());
    int newY = static_cast<int>(rotatedRect.top());
    int newW = static_cast<int>(rotatedRect.width());
    int newH = static_cast<int>(rotatedRect.height());

    specialWidgetUpdate(newX, newY, newW, newH);
    if (rotationAngle % 180)
        setGeometry(newX, newY, newH, newW);
    else
...
void Chart_Base::updateSizePointInfo()
{
    if (sizePoint.i_point.size() >= 4) {
        QPointF topLeft(paintStart.rx(), paintStart.ry());
        QPointF topRight(paintEnd.rx(), paintStart.ry());
        QPointF bottomRight(paintEnd.rx(), paintEnd.ry());
        QPointF bottomLeft(paintStart.rx(), paintEnd.ry());
        ...
        sizePoint.i_point[0]->setX(topLeft.x());
        sizePoint.i_point[0]->setY(topLeft.y());
        sizePoint.i_point[0]->setRotate(ORIENTION::NORTHWEST);

        sizePoint.i_point[1]->setX(topRight.x());
        sizePoint.i_point[1]->setY(topRight.y());
        sizePoint.i_point[1]->setRotate(ORIENTION::NORTHEAST);

        sizePoint.i_point[2]->setX(bottomRight.x());
        sizePoint.i_point[2]->setY(bottomRight.y());
        sizePoint.i_point[2]->setRotate(ORIENTION::SOUTHEAST);

        sizePoint.i_point[3]->setX(bottomLeft.x());
        sizePoint.i_point[3]->setY(bottomLeft.y());
...
void Chart_Base::updateMagPointInfo()
{
    if (magPoint.i_point.size() >= 4) {
        ...
        int midx = (paintStart.rx() + paintEnd.rx()) / 2;
        int midy = (paintStart.ry() + paintEnd.ry()) / 2;
...
void Chart_Base::paintEvent(QPaintEvent *event)
{
    ...
    if (showAll) {
        paintSizeEdge(painter);
        paintMagPoint(painter);
        paintSizePoint(painter);
    } else if (showMag) {
        paintMagPoint(painter);
    }
...
void Chart_Base::textInit()
{
    ...
    if(chartText.textType)
    {
        ...
        chartText.textType1->setText("请输入");
        chartText.textType1->setWordWrap(true);
        chartText.textType1->setAlignment(Qt::AlignCenter);  // 文字水平和垂直居中
        chartText.textType1->adjustSize(); // 先调整文本框的大小
        qDebug() << chartText.textType1->width();
        qDebug() << chartText.textType1->height();

        connect(chartText.textType1, SIGNAL(setTypeChangeTextPos(CHART_LABEL_MOUSE_TYPE, int, int)),
                this, SLOT(setTypeChangeTextPos(CHART_LABEL_MOUSE_TYPE, int, int)));

        connect(chartText.textType1, &Chart_Label::setTypeChangeTextPos, this, [=]() {
            emit changeText();
        });
    ...
    else
    {
        ...
        chartText.textType2->setPlainText("请输入");
        ...
        QTextOption option = chartText.textType2->document()->defaultTextOption();
        option.setAlignment(Qt::AlignCenter);  // 设置文字水平和垂直居中
        chartText.textType2->document()->setDefaultTextOption(option);
        chartText.textType2->adjustSize(); // 先调整文本框的大小

        qDebug() << chartText.textType2->parentWidget(); // 父控件
        qDebug() << chartText.textType2->layout();       // 布局管理器

        chartText.textType2->setSizeAdjustPolicy(QAbstractScrollArea::AdjustToContents);
        chartText.textType2->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
        chartText.textType2->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
        chartText.textType2->setStyleSheet("background-color : transparent");
        chartText.textType2->move(parentwidth/2 - chartText.textType2->width()/2,parentheight/2-chartText.textType2->height()/2);
        chartText.textType2->adjustSize();
        connect(chartText.textType2, SIGNAL(setTypeChangeTextPos(CHART_TEXT_MOUSE_TYPE, int, int)),
                this, SLOT(setTypeChangeTextPos(CHART_TEXT_MOUSE_TYPE, int, int)));

        connect(chartText.textType2, &Chart_PlainText::setTypeChangeTextPos, this, [=]() {
            emit changeText();
        });
...
{
    chartText.textType2->move(width()/2 - chartText.textType2->width()/2,height()/2-chartText.textType2->height()/2);
