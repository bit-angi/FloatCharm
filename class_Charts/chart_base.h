    ...
    public:
        ...
        friend QDataStream &operator<<(QDataStream &fout,  const TextBase &tb)
        {
            ...
            if(tb.textType)
            {
                ...
                fout<<ql->geometry()<<ql->text().toUtf8();
                ...
                QTextEdit * qpte = tb.textType2;
                fout<<qpte->geometry()<<qpte->toPlainText().toUtf8();
        ...
        friend QDataStream &operator>>(QDataStream &fin, TextBase &tb)
        {
            ...
            if(tb.textType)
            {
                ...
                qDebug() << "tmp : " << QString(tmp) << "len " << QString(tmp).length();
                ql->setText(QString(tmp).left(QString(tmp).length()-1));
                ...
                fin.readRawData(tmp.data(),len);
                qDebug() << "tmp : " << QString(tmp) << "len " << QString(tmp).length();
                qpte->setPlainText(QString(tmp).left(QString(tmp).length()-1));
